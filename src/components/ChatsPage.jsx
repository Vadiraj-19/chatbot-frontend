...
export default function ChatsPage() {
  ...
  const { data, loading, error } = useQuery(CHATS, { fetchPolicy: "network-only" });
  ...
  const [createChat] = useMutation(CREATE_CHAT, {
    update(cache, { data: { insert_chats_one } }) {
      ...
      if (existing) {
        cache.writeQuery({
          query: CHATS,
          data: { chats: [insert_chats_one, ...existing.chats] },
        });
      }
  ...
  const handleCreateChat = () => createChat({ variables: { title: "New Chat" } });
  ...
  return (
    <div className="h-screen w-screen flex bg-black text-white overflow-hidden">
      ...
      <aside
        ...
        className={`fixed top-0 left-0 z-50 h-full w-72 xl:w-96  bg-neutral-900 border-r border-neutral-800 flex flex-col transition-transform duration-300 ease-in-out ${
          ...
          {data?.chats?.map((chat) => (
            <li
              ...
              className={`flex items-center justify-between px-3 py-2 rounded-md cursor-pointer truncate text-sm transition-all ${
                selectedChat === chat.id ? "bg-neutral-800 text-white" : "text-gray-400 hover:bg-neutral-800/70"
              }`}
              onClick={() => {
      ...
      <main className="flex-1 min-h-0 flex flex-col bg-gradient-to-b from-neutral-950 to-black">
        {/* Mobile header with hamburger and branding */}
        <header className="flex items-center justify-between bg-neutral-900 p-4 border-b border-neutral-700 sm:hidden">
          ...
          <div className="flex items-center gap-2">
            <img src={Logo} alt="Vynt Logo" className="w-8 h-8 select-none" draggable={false} />
            <span className="text-lg font-bold tracking-wide">Vynt</span>
          </div>
        ...
        {selectedChat ? (
          ...
          <div className="flex items-center justify-center h-full text-gray-500 text-base">Select a chat to start</div>
        )}
      </main>
    </div>
  );
